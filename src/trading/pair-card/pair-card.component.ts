...
export class PairCardComponent implements OnInit, OnDestroy, AfterViewInit {
  @Input() pair!: string;

  @Output() remove = new EventEmitter<string>();

  public panelOpenState = false;
  public ticker?: TickerInfo;
  ...
  ngOnInit(): void {
    ...
    this.facade.ticker(this.pair).subscribe((ticker) => {
      ...
      this.priceDown = this.ticker
        ? Number(this.ticker.change_percentage) < 0
        : false;
  ...
  public get isRed() {
    return (
      ...
      Number(this.ticker.last) < this.averages.buy.price
    );
  }
  ...
  private updateHeaderColor() {
    ...
    if (this.isRed) {
      const greenBlue =
        250 -
        Math.floor(
          ((this.averages!.buy.price - Number(this.ticker!.last)) /
